...
function MovieTable(props) {
  return (
    <table className="movies-table">
      ...
      <tbody>
        {props.children.map((movie, index) => <tr key={index}>
          <td><Link to={{
            pathname: `/movie/${movie.id}`,
            state: { modal: true }
          }}>{movie.title} {movie.sponsored &&
            <span className="badge badge-info">Sponsored</span>}</Link></td>
          <td>{movie.year}</td>
          ...
          <td><a target="_blank" rel="noopener noreferrer"
            ...
            IMDB <i
              ...
              className="material-icons">open_in_new</i></a></td>
        </tr>)}
      </tbody>
    </table>
  )
